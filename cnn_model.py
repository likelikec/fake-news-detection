...
from keras.layers import Dense, Embedding, Activation, Concatenate,Input, Lambda, Reshape
...
if __name__ =='__main__':
    ...
    plt.plot(history.history['accuracy'])  # 绘制训练准确率
    plt.plot(history.history['val_accuracy'])  # 绘制验证准确率
    plt.legend(['training', 'validation'], loc='upper left')  # 修正了这里的文本
    plt.xlabel('Epoch')  # 添加x轴标签
    plt.ylabel('Accuracy')  # 添加y轴标签
    plt.title('Training and Validation Accuracy')  # 添加图表标题
    plt.show()
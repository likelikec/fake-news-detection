...
def get_tfidf_data():#jieba+tfidf生成词向量，X为2d,仅用title
    ...
    # 初始化tokenized_corpus列表
    tokenized_corpus = []
    for text in data['Title']:
        tokenized_corpus.append(" ".join(jieba.cut(text)))# 对每个标题进行分词处理，并将分词结果添加到tokenized_corpus列表中
    Y=data['label']
    tfidf = TfidfVectorizer(analyzer='word')
    X= tfidf.fit_transform(tokenized_corpus)

    print(tfidf.vocabulary_)
    return X.A,Y

def get_wordVec2d_data():#word2vec词向量，X为2d,用title
    X,Y=get_wordVec3d_data()
    X=np.array([i.flatten() for i in X])
    return X,Y

import os
def get_wordVec3d_data():#从文件中输出word2vec词向量，X为3d（总数据数，max（每句中单词数），300=embeding维数）
